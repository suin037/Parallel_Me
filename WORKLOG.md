...
6. **쉬어가기(break) 소득효과에 '쉬는 동안'이 안 들어 있다.** (2026-08-17 확인)

   제보: "반년 쉬고 복귀했는데 그냥 남는 것보다 소득이 더 높게 나온다."
   배포된 `/compare` 를 도현(31세 남, 연구·공학, 420만)의 실제 선택지로 호출해 재봤다.

   | 연차 | `dynamic_effects` ATE | API 실제 차이(A−B) |
   |---|---|---|
   | 1년 | +17.97 | 318.0 − 300.0 = **+18.0** |
   | 3년 | −1.09 | 332.9 − 334.0 = **−1.1** |
   | 5년 | +18.60 | 369.6 − 351.0 = **+18.6** |

   **배선은 정상이다.** 연차별 ATE 가 그대로 적용된다. 문제는 결과변수 정의다 —
   `train_treatments.py` 의 `outcome="월소득_실질"` 은 `t+horizon` 시점의 **월임금**을
   읽는다(`out_wage = g[outcome].shift(-horizon)`). KLIPS 공백 스펠로 학습한 효과는
   **복귀한 뒤의 임금**을 견주므로, 공백 기간의 0원이 결과변수에 애초에 없다.

   그래서 같은 데이터로 재학습해도 똑같이 나온다. 고치려면 결과변수를 바꿔야 한다:
   - 연간 총소득(공백 개월의 0 포함) 또는 공백 포함 평균 월소득
   - 또는 A/B 의 3년 누적 소득 격차로 표시

   추가로 이 +18만원 자체가 선택편향일 수 있다 — 쉴 수 있는 사람이 원래 더 좋은
   자리로 복귀한다. 6절의 "L1 이직자 소득 +28% 는 선택편향" 과 같은 종류다.

   **지금 조치**: 숫자는 그대로 두고, 무엇이 빠졌는지를 결과 화면
   (`ResultDataNotes` 의 "쉬는 동안의 소득은 이 비교에 없습니다")에 밝혔다.

   **재학습 준비물은 확보됐다** — `data/raw/klips/` 에 배치 완료:
   `klips_break.pkl`(7MB), `klips27w.sav`(88.7MB, 직업력 원본·누적이라 이것 하나면 된다).
   `preprocess_klips_jobhist.py` 가 정상 인식하는 것까지 확인했다.
...
- [ ] KLIPS 전처리 **코드북 검증**
- [x] ~~실질임금 디플레이트~~ — 완료(2024년 기준). `klips_build_report.json` 의
      `deflated: true / cpi_base_year: 2024`. 그 파일이 배포에 없어서 앱이 소득을
      "명목(기준연도 미상)"으로 표시하던 문제도 함께 해결(2026-08-17)
...
- [ ] **쉬어가기 결과변수 재정의 후 `econml_klips_break` 재학습** — 공백 기간의 0원이
      들어가게(연간 총소득 등). 재학습만으로는 안 바뀐다 (7-6 참고)
