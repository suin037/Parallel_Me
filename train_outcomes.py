...
OUTCOMES = {
    ...
    "근로시간만족": {"label": "근로시간 만족", "unit": "점(1~5)", "scale": "likert5",
                     "higher_is_better": True, "note": "취업자만 응답"},
    # 관계 3종 — 커리어 선택의 '관계 비용'을 재는 축. 생활만족 배터리(p{w}6504~6506)라
    # 취업 여부와 무관하게 전체가 응답하므로, 직무만족과 달리 t+h 에 일하지 않아도
    # 결과가 관측된다. 창업·쉬어가기처럼 비취업 상태를 지나는 처치에서 특히 중요하다.
    "가족관계만족": {"label": "가족관계 만족", "unit": "점(1~5)", "scale": "likert5",
                     "higher_is_better": True, "note": "전체 응답자"},
    "친인척관계만족": {"label": "친인척관계 만족", "unit": "점(1~5)", "scale": "likert5",
                       "higher_is_better": True, "note": "전체 응답자"},
    "사회관계만족": {"label": "사회적 친분관계 만족", "unit": "점(1~5)", "scale": "likert5",
                     "higher_is_better": True,
                     "note": "전체 응답자. 가족 밖 관계를 재는 유일한 축"},
}
DEFAULT_OUTCOMES = ["월소득_실질", "직무만족", "생활만족", "건강",
                    "가족관계만족", "친인척관계만족", "사회관계만족"]
