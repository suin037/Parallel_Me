...
function SideRisk({ result }) {
  ...
  return (
    <Card>
      ...
      <Caption>
        {/* result.choice 는 사용자가 자유롭게 적은 원문이라("퇴사하고 다른 제조업체로
            이직한다") "이직" 과 문자 그대로 같을 일이 거의 없다 — 항상 세 번째
            가지(창업 문구)로 떨어져, 이직·휴식 케이스에도 "폐업한 비율" 이 붙었다.
            분류된 유형은 result.kind 에 있으니 그걸 봐야 한다. */}
        {result.kind === "이직"
