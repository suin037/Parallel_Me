...
//
// 조건이 하나 더 있다 — **backend/choice_classifier.py 도 읽을 수 있어야 한다**
// (아래 RELATED_ALTERNATIVES 주석 참고). 영역 태그만 맞고 유형이 '기타' 면
// 처치가 안 붙어 L3·L4 가 꺼진다. 그래서 문구를 고칠 때 둘 다 확인해야 한다:
//   (1) 자기 영역 키워드가 남아 있고 다른 영역이 새로 붙지 않는가
//   (2) 분류기가 유형을 읽는가
const COMPARE_PROMPTS = {
  ...
  health: { a: "치료나 회복을 우선해 잠시 쉬어가기", b: "지금 일상을 유지하며 작은 건강 습관부터 만들기" },
  housing: { a: "새로운 지역이나 집으로 이사하기", b: "현재 거주지를 유지하며 조건을 개선하기" },
  relationship: { a: "관계에 변화를 주고 솔직하게 대화하기", b: "잠시 거리를 두고 관계를 다시 살펴보기" },
  lifestyle: { a: "일과 생활의 균형을 크게 바꾸기", b: "지금 생활을 그대로 두고 작은 변화를 시험해보기" },
  long_term_values: { a: "지금 중요한 가치를 따라 방향을 정하기", b: "지금 상태를 유지하며 장기적인 가능성을 더 확인하기" },
};

// A에 적은 길과 같은 문제를 다른 방식으로 풀 수 있는 B 후보들.
// 영역마다 하나만 보여주던 기존 방식과 달리, 같은 맥락 안에서 유지·조정·유예·시험을 비교한다.
//
// ⚠ 여기 문구는 **backend/choice_classifier.py 가 읽을 수 있어야 한다.**
//   앱이 권한 문구를 사용자가 그대로 눌렀는데 '기타'로 분류되면 처치 매핑이 안 붙어
//   L3(인과)·L4(생존)가 꺼진다. 근거를 갖고 답하려고 만든 화면에서 근거가 사라지는 셈이다.
//   실제로 36개 중 22개가 기타였다.
//
//   유지 단서: 남아·그대로·유지·계속 다니·버티   |  휴식: 쉬어가·잠시 쉬·휴직
//   이직: 옮기·옮겨·다른 회사                    |  창업: 창업·차리·내 브랜드
//   부정은 키워드 **뒤** 6글자를 본다("창업 대신" → 창업이 물린다).
//
// 고칠 때 **두 가지를 같이** 봐야 한다. 분류 단서만 보고 고치다가 위 LIFE_DOMAINS
// 키워드를 빼먹어 영역 태그가 빈 적이 있다("학위 대신…"에서 '학위'를 지웠고,
// "다른 지역보다…"를 '생활권'으로 바꿔 주거가 생활방식으로 넘어갔다).
//   (1) 분류기가 유형을 읽는가        (2) 자기 영역 키워드가 남아 있는가
//
// 36개 중 22개가 기타였고, 지금은 9개다. 남은 9개는 고치지 않은 게 아니라 그게 맞는 자리다:
//   · finance 3개 · long_term_values 2개 — 지출 줄이기·소액 시험·소득 흐름·가치
//     우선하기·되돌릴 수 있는 선택. 학습된 처치(이직·창업·휴식·결혼·주택·이사)에
//     대응이 없다. 단어를 바꿔도 L3/L4 는 안 붙고, 억지로 '유지'로 만들면 하지도
//     않은 결정을 했다고 답하게 된다.
//   · relationship 3개 — 분류기에 관계 유형 자체가 없다(설계상). 대신 프론트의
//     scenarioIntake.EVENT_RULES 가 '솔직한 대화'·'거리두기'를 사건으로 잡는다.
//   · "바로 이사하지 않고 단기로 살아보기" — 부정을 제대로 읽은 결과다(이사(-)).
//     단어를 바꾸면 오히려 '이사한다'로 뒤집힌다.
const RELATED_ALTERNATIVES = {
  career: ["현재 직장에 남아 조건을 조정하기", "지금 회사에 남아 직무나 팀을 바꿔보기", "이직을 미루고 필요한 역량부터 준비하기", "휴직이나 근무 형태 변경을 먼저 알아보기"],
  education: ["현업을 유지하며 공부를 병행하기", "진학을 미루고 단기 과정부터 들어보기", "현업을 유지하며 학위 대신 실무 경험을 쌓기", "지금 일을 그대로 하며 수업 하나로 먼저 시험해보기"],
  business: ["직장을 유지하며 사이드 프로젝트로 사업성을 시험하기", "직장에 남아 고객 반응부터 확인하기", "동업자와 함께 창업하기", "창업 대신 관련 회사로 옮겨 경험을 쌓기"],
  finance: ["수입 확대보다 지출과 위험을 먼저 줄이기", "큰 결정을 미루고 소액으로 시험하기", "현재 계획을 유지하며 비상자금을 더 만들기", "큰 수익보다 매달 들어오는 소득 흐름을 우선하기"],
  health: ["현재 일상을 유지하며 작은 회복 습관부터 만들기", "혼자 버티기보다 치료나 상담의 도움을 받아보기", "일정을 줄이고 잠시 쉬어가는 기간을 갖기", "지금 강도를 유지하며 지속 가능하게 조정하기"],
  housing: ["지금 집에 그대로 머물며 불편한 조건을 개선하기", "바로 이사하지 않고 단기로 살아보기", "이사 시점을 뒤로 미루고 자금을 먼저 모으기", "지금 거주지에 남아 다른 조건을 찾아보기"],
  relationship: ["바로 결론내리지 않고 솔직하게 대화해보기", "잠시 거리를 두고 관계를 다시 확인하기", "관계를 유지하되 경계와 조건을 분명히 하기", "주변이나 전문가에게 객관적인 의견을 구하기"],
  lifestyle: ["지금 생활을 그대로 두고 작은 변화부터 시험하기", "현재 루틴을 유지하며 시간 배분만 조정하기", "지금 방식을 유지하며 일정 기간만 새 방식을 체험해보기", "포기할 것과 유지할 것을 나눠 단계적으로 바꾸기"],
